...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method before = null;
        for (Class<?> clazz = pageClass; before == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                before = clazz.getDeclaredMethod("before", HttpServletRequest.class, Map.class);
            } catch (NoSuchMethodException ignored) {
                // No operations.
            }
        }
        ...
        Method after = null;
        for (Class<?> clazz = pageClass; after == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                after = clazz.getDeclaredMethod("after", HttpServletRequest.class, Map.class);
            } catch (NoSuchMethodException ignored) {
                // No operations.
            }
        }
        ...
        try {
            ...
            throw new ServletException("Unable to run action [pageClass=" + pageClass + ", method=" + after + "].", e);
        } catch (InvocationTargetException e) {
            Throwable cause = e.getCause();
            if (cause instanceof RedirectException) {
                RedirectException redirectException = (RedirectException) cause;
