...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        Set<Class<?>> acceptableTypes = new HashSet<>();
        acceptableTypes.add(HttpServletRequest.class);
        acceptableTypes.add(Map.class);

        Method method = null;
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            for (Method m : clazz.getDeclaredMethods()) {
                if (m.getName().equals(route.getAction())
                        && Arrays.stream(m.getParameterTypes()).allMatch(acceptableTypes::contains)) {
                    method = m;
                    break;
                }
            }
        ...
        if (session.getAttribute("lang") != null && !session.getAttribute("lang").equals("en")) {
            try {
                Template template = newTemplate(pageClass.getSimpleName() + "_" + session.getAttribute("lang") + ".ftlh");
                response.setContentType("text/html");
                response.setCharacterEncoding(StandardCharsets.UTF_8.name());
                template.process(view, response.getWriter());
            } catch (ServletException | TemplateException e) {
